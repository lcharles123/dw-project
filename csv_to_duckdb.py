...
def import_csv(csv_files):
    ...
    for csv in csv_files:
        if csv['columns'] == 27:
            conn.execute(f"""
            CREATE TABLE IF NOT EXISTS atendimentoporfonecedor AS
            SELECT * FROM read_csv('{csv["link"]}', delim='{csv["delim"]}', all_varchar=true, ignore_errors=true) LIMIT 0;
            
            INSERT INTO atendimentoporfonecedor
...
csv_dir = '/root/dw/dbt_project/imported_raw_data/oper_csv_files/'
...
# Verify results
print("\nFinal table schema:")
print(conn.sql("DESCRIBE atendimentoporfonecedor"))
print(conn.sql("SELECT COUNT(*) FROM atendimentoporfonecedor"))
